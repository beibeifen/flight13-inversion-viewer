...
这些输入把无尺度图形映射到吨位；不是视频独立反演出的绝对质量。页面中由此得到的“当前质量”“推进剂跨度”和质量曲线，都是四个输入与圆弧指标的前向计算，不能作为对这些输入的独立验证。

旧板块曾包含“由上述条件质量再计算推力”的展示；该链路对独立质量反演没有信息增益，现已从页面、计算代码和公式中删除。旧板块现在只保留圆弧到条件质量的前向映射。

现有质量板块下方另设“额定推力假设下的动力学质量反演”板块。模型直接使用条件三维轨迹的位置二阶导数作为 ECEF 加速度，再结合额定推力、重力、地球自转修正和显式气动假设求质量正根；时间对齐、推力和阻力扰动形成工程敏感性范围。新模型不读取旧四参数质量。

公开演示包运行相同的公式和交互代码，但输入全部是 `scripts/build_public_demo.py` 生成的合成轨迹、合成二阶导数和合成发动机门控，只用于验证计算链，不构成 Flight 13 质量估计。完整公式和适用边界见 [docs/METHODOLOGY.md](docs/METHODOLOGY.md)。
